...
//				'jump' => U('Goods/index')
	...
	public function show_goods_page($search)
	{
		...
		if (isset($search['status'])) {
			if($search['status'] == 0){
				$sql .= " and pd.shangjia=" . 5555;
			}else{
				$sql .= " and pd.shangjia <> 0";
			}
...
//		$goods['is_hot'] = $data['hot'];
		$goods['gr_hot'] = $data['hot'];
//		$goods['is_new'] = $data['new'];
		$goods['gr_new'] = $data['new'];
		$goods['is_duobao'] = 1;
//		$goods['is_tuijian'] = $data['tuijian'];
		$goods['gr_tuijian'] = $data['tuijian'];
		//消费使用比例
...
//		$goods['old_price'] = $data['old_price'];
		...
		$goods['freight'] = $data['freight'];
		$goods['address'] = $data['address'];
		$goods['is_sort'] = intval($data['sort']);
//		$goods['is_hot'] = $data['hot'];
//		$goods['is_new'] = $data['new'];
//		$goods['is_tuijian'] = $data['tuijian'];
		$goods['gr_hot'] = $data['hot'];
...
//		$goods['jifen_nums'] = $data['jifen_nums'];
		...
		$goods['shangjia'] = 5555;//后台商城商品指定账号收款
		$goods['ctime'] = time();
		//夺宝产品还是普通产品
//		dump($goods);exit;
