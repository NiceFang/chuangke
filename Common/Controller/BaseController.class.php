 ...
 * @author andery
 */
namespace shop\Common\Controller;
...
// echo $statusStr[$result];
	...
	function setLevel($level,$ge,$teamlevel,$teamge,$sh1,$sh2,$id,$parentpath){
		...
		$defaultuser = M('users')->where("id =1 ")->order("id desc")->find();
			...
			$tjcount = M('users')->where("rid='{$id}' ".$wheresql)->count();
			$wheresql = $this->SysSet[$teamlevel]==-1  ? "" : " and standardlevel=".$this->SysSet[$teamlevel];
			$teamtjcount = M('users')->where("FIND_IN_SET($id,rpath) ".$wheresql)->count();
			...
			if($tjcount >= $this->SysSet[$ge] && $teamtjcount>= $this->SysSet[$teamge]){
				...
				if($this->SysSet[$sh1]==-1){
					...
					$returnArray["find1"] = M('users')->where("id in ($parentpath) ".$wheresql)->order("id desc")->find();
				...
				if($this->SysSet[$sh2]==-1){
					...
					$returnArray["find2"] = M('users')->where("id in ($parentpath) ".$wheresql)->order("id desc")->find();
