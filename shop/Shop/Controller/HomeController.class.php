...
/* var_dump($photo);die;*/
      if(empty($data['username'])){
            echo '<script>alert("请输入充值Rtoken账号"); window.history.back(-1); </script>';
...
public function cation()
	...
	public function index()
	{
        ...
        //总后台商品还是个人店铺商品，0为总后台
        $shang['shangjia']=array('neq',0);
	    //火爆产品
	    $hot_product_list = M("product_detail")->where(array("gr_hot"=>1))->where($shang)->limit(6)->order("ctime desc")->select();
	    $this->assign("hot_product_list",$hot_product_list);

        //最新产品
        $new_product_list = M("product_detail")->where(array("gr_new"=>1))->where($shang)->order("ctime desc")->limit(7)->select();
        ...
        //为您推荐
        $like_product_list = M("product_detail")->where(array("gr_tuijian"=>1))->where($shang)->order("ctime desc")->limit(8)->select();
...
//        $data['s_desc'] = I('s_desc');
        ...
        // $data['jifen_nums'] = I('jifen_nums');//赠送矿池数量
