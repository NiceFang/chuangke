	...
	public function index()
	{
		...
		if ($fromtype == 99) {
            ...
            foreach ($OrderId as $key=>$val){
                $val =(int)$val;
                $price +=floatval( M("order")->where("order_id=$val")->field('buy_price')->find()['buy_price']);//计算订单总价格
                $order[] = M("order")->where("order_id=$val")->find();

            }

           /* foreach($all_oid as $val){
                $orderinfo = $order_db->where("order_id=$val")->select();
            }*/
            ...
            //个人订单过来的
            $price = '';
            $price +=floatval( M("order")->where("order_id=$all_oid")->field('buy_price')->find()['buy_price']);//计算订单总价格
            $order[] = M("order")->where(array("order_id" => $all_oid))->find();
//			$order = $order_db->where(array("order_id" => $all_oid))->select();
		...
		//查是否有足够的金额
		$farmlink_jifen = M("store")->where("uid=$uid")->getField("cangku_num");
...
//	{
//		//$type = trim(I("type"));//状态id
//		//	$oid = trim($_GET['oidd']);//订单id
//
////		var_dump($po);exit;
//		//	$oidd[] = I('oidd');  //获取
////		dump($oidd);exit;
//		$oid = session('Order_id');
//		$pay_money = $_GET['money'];
//		if(empty($oid)){
//			$this->error("参数有误，请联系管理员",'/Shop/Home/index');
//		}
////		dump($oid);exit;
////		var_dump($pay_money);exit;
//		$uid = session('userid');//用户id
//		!$uid && error_alert("请登录后进行操作");$oid || error_alert("缺少订单参数");
//		//	$oidd || error_alert('缺少订单参数');
//
//		//订单验证
//		foreach ($oid as $key => $val ){
//			$val = (int)$val;
//			$order = M("order")->where(array("order_id" => $val, "uid" => $uid, "status" => 0))->find();
//			if (!$order) {
...
//
////            //添加释放记录
////            userAward($uid,'zhuandj1',3,$pay_money);
//
//			//查询当前P矿池
...
//			addAccountRecords($uid,0,$yin_jifen,31,$userAcount['fengmi_num'],'scores');
...
//				try{
//					foreach ($oid as $val){
//						$zhifu = M('order')->where("order_id=$val")->save($arr);
//						if(!$zhifu){
//							M()->rollback();//回滚
//							ajaxReturn('支付失败请联系管理员', 0);
//						}
//						$orderTable = M('order_detail')->where("order_id=$val")->save($data);
//						if(!$orderTable){
//							M()->rollback();//回滚
//							ajaxReturn('支付失败请联系管理员', 0);
//						}
//					}
//					M()->commit();//提交
//					ajaxReturn('支付成功', 1,'/Shop/Member/allorder/s/1.html');
//				}catch (\Exception $ex){
//					M()->rollback();//回滚
//					ajaxReturn('支付失败请联系管理员', 0);
//				}
...
//		$oidd[] = I('oidd');  //获取
        ...
        $pay_money = $_GET['money'];
		$uid = session('userid');//用户id
		!$uid && ajaxReturn("请登录后进行操作");
//		$oid || error_alert("缺少订单参数");
		$oidd || ajaxReturn('缺少订单参数');
		//订单验证
        foreach ($oidd as $key => $val ){
            $val = (int)$val;

            $order = M("order")->where(array("order_id" => $val, "uid" => $uid, "status" => 0))->find();
//            ajaxReturn(!$order);
            if (!$order) {
                ajaxReturn("该订单不存在或已支付");
            }
            if (!empty($order['order_proof'])) {
                ajaxReturn("该订单已支付");
            }
...
//			$body = $_POST['WIDbody'];
	...
	protected function jfpay($order_no, $pay_type, $jifentypes)
	{
		...
		if ($pay_type == 3) {//只使用农联矿池
			if ($farmlink_jifen < $pay_money) {
				$data['status'] = "0";
				$data['info'] = "农联矿池不足，无法支付。";
				return $data;
			}
			//根据矿池类型给用户加上对应赠送矿池
			if ($jifentypes == 1) {
