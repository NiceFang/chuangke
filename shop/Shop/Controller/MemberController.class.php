...

{
...

	{
		...
		$s = I("s");
		$findwhere['o.uid'] = $uid;
		$findwhere['o.is_show'] = 1;
		if ($s >= 0 && $s < 4 ) {
			$findwhere['o.status'] = $s;
		}


		$orderList = M("order o")
            ->field('o.order_id,o.status,o.order_no,o.time,o.buy_phone,gr.shop_name')
            ->join('ysk_gerenshangpu gr ON o.order_sellerid = gr.id','LEFT')
            ->where($findwhere)->order('order_id desc')->select();


		foreach ($orderList as $key => $order) {
			$deid = $order["order_id"];
			$orderList[$key]["detail"] = M("order_detail")->where(array("order_id" => $deid))->select();
            $orderList[$key]['time'] = date('Y-m-d H:i:s',$order['time']);
		}
    //    exit;
		//D('product_detail')
...
//		$detailadd = I('detailadd');
		...
		//判断收货地址是否存在
		$hasAddress = D('address')->where(array('menber_id' => $uid,'name' => $uanme,'telephone' =>$phone,'province_id'=>$province,'city_id'=>$city,'country_id'=>$district,'address'=>$address))->find();
		if($hasAddress){
			ajaxReturn('收货地址已存在',0);
		}
...
//			var_dump($data);exit;
...

	{
		...
		if ($txtype == 4) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['ji_tixian_xiaofe'] + 0; //消费矿池
		...
		if ($txtype == 5) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['yu_tixian_xiaofe'] + 0; //消费矿池
		...
		if ($txtype == 6) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['guo_tixian_xiaofe'] + 0; //消费矿池
		...
		if ($mine_jiefn < $txnums) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,您暂未拥有这么多矿池');
		...
		if ($txnums < $withconfigs['tixian_start']) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,最低起提矿池是' . $withconfigs['tixian_start'] . '');
		...
		if ($txtype <= 3) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['tixian_xiaofei_yj'] + 0; //消费矿池
		...
		$data['give_xiaofei'] = $jifen_xiaofei;//拆分出来的消费矿池
...

	{
		...
		if ($res_change) {
			...
			$moneys=bcmul($money,0.8,5);
			$jifeng = bcmul($money,0.2,5);
//			$allData = array('cangku_num'=>$money,'fengmi_num'=>$jifeng);
           ...
           // formatLevel($uid,$userInfo['use_grade'],$userInfo['is_degraded']);
			...
			if($res && $resjifeng){
                //添加释放记录
                userAward($datas['uid'],'zhuandj1',3,$money);
