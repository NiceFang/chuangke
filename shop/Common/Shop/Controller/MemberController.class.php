...

{
...

	{
		...
		if ($txtype == 4) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['ji_tixian_xiaofe'] + 0; //消费
		...
		if ($txtype == 5) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['yu_tixian_xiaofe'] + 0; //消费
		...
		if ($txtype == 6) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['guo_tixian_xiaofe'] + 0; //消费
		...
		if ($mine_jiefn < $txnums) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,您暂未拥有这么多');
		...
		if ($txnums < $withconfigs['tixian_start']) {
			...
			$mes = array('status' => 2, 'info' => '提现失败,最低起提是' . $withconfigs['tixian_start'] . '');
		...
		if ($txtype <= 3) {
			...
			$jifen_xiaofei = $txnums * $withconfigs['tixian_xiaofei_yj'] + 0; //消费
		...
		$data['give_xiaofei'] = $jifen_xiaofei;//拆分出来的消费
...

	{
		...
		if ($res_change) {
			...
			$userId = $datas['uid'];
			$userInfo = $user->field('is_center')->where(array('userid' => $userId))->find();
            $sellInfo = $user->where(array('userid' => $uid))->Field('use_grade,is_degraded,is_center')->find();
            $money=$datas['buy_price'];

			if($userInfo['is_center'] == 1 || $sellInfo['is_center'] == 1){
                ...
                //添加余额记录
                addAccountRecords($uid,$datas['uid'],$money,32,$account['cangku_num'],'money');
            }else{
