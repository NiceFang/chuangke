...
function login(){
    ...
    var zhbnwk = $('#zhbnwk').val();//账号不能为空
    var mmbnwk = $('#mmbnwk').val();//密码不能为空
	  if(account=='' || account==null){
	  	msg_alert(zhbnwk);
      return false;
	  }
...
function adduser(){
    ...
    var jymmznw6w = $('#jymmznw6w').val();//交易密码只能为6位数
    var yzmbnwk = $('#yzmbnwk').val(); //验证码不能为空
    var xmbnwk = $('#xmbnwk').val();  //姓名不能为空
    var sjhbnwk = $('#sjhmbnwk').val();//手机号码不能为空
    var dlmmbnwk = $('#dlmmbnwk').val();//登录密码不能为空
    var lcsrdlmmbyz = $('#lcsrdlmmbyz').val(); //两次登录密码不一致
    var reg = /^\d{6}\b/;
...
function SetPwd(){
    ...
    var qsrsjhm = $('#qsrsjhm').val(); //请输入手机号码
    var yzmbnwk = $('#yzmbnwk').val(); //验证码不能为空
    var qsrxmm = $('#qsrxmm').val(); //请输入新密码
    var qsrqrmm = $('#qsrqrmm').val(); //请输入确认密码
    var lcsrdmmbyz = $('#lcsrdmmbyz').val();//两次输入的密码不一致
// console.log(qsrsjhm);
// alert(12312);
    if(mobile=='' || mobile==null){
      layer.msg(qsrsjhm);
