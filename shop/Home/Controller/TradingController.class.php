...
//卖出记录
    ...
    //        $res_pay = M('store')->where(array('uid'=>$order_info['payin_id']))->setInc('cangku_num',$paynums);//转入的人+80%
    ...
    public function Dosells(){
        if(IS_AJAX){
            ...
            if($sellnums['pay_state'] != 0){
                ajaxReturn('对不起，有人先一步交易了',0);
            }
            //验证点击的是否为有人买入
            $saled = D('trans')->where(['id'=>$trid])->getField('pay_state');
            if($saled > 0){
                ajaxReturn('对不起，有人先一步交易了',0);
            }
            //验证交易密码
...
//            var_dump($verifyDate);
            ...
            $shouxufe = M('store')->where(array('uid'=>$uid))->setDec('cangku_num',$shouxufei);
            $pay_n = M('store')->where(array('uid' => $uid))->getfield('cangku_num');
            //添加余额记录
            addAccountRecords($uid,0,-$shouxufei,23,$pay_n,$type = 'money');
//                $jifen_dochange['now_nums'] = $pay_n;
...
//                $res_addres = M('tranmoney')->add($jifen_dochange);
                ...
                //记录买入会员
                $res_Buy = M('trans')->lock('true')->where(array('id'=>$trid))->setField(array('payout_id'=>$uid,'pay_state'=>1,'card_id'=>$id_setcards['id'],'fee_nums'=>$shouxufei));
                if($res_Buy && $doDec){
                    M()->commit();
                    ajaxReturn(L('mccg'),1);
                }else{
