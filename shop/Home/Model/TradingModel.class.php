...
class TradingModel extends ModelModel
{
   ...
   protected function getMobile(){
    ...
    /**
     * @param $pay_type 支付方式
     * @param $trans_type 0买入1卖出
     * @param $uid userid
     */
    public function hasOrder($pay_type,$uid){
        $data = D('trans')->where(['pay_type'=>$pay_type,'pay_state'=>['in','0,1']])->select();
        $result = [];
//        dump($data);
       foreach ($data as $key=>$value){
           if($value['trans_type'] == 0 && $value['payin_id'] == $uid){
               $result = $value;
           }elseif ($value['trans_type'] == 1 && $value['payout_id'] == $uid){
                $result = $value;
           }
       }
//       dump(empty($result));
