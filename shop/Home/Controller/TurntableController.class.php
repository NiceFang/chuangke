...
//    }
    public function index(){
        ...
        //用户icc币的检测
        $iccTest = M('ucoins')->where(['c_uid'=>$uid,'cid'=>20925])->find();
//        dump($iccTest);exit;
        if($iccTest === null){
            $data['c_uid'] = $uid;
            $data['c_nums'] = 0;
...
//防重复提交
    ...
    session("gou_last_time", $t);
        ...
        //添加余额记录
        addAccountRecords($sell_id,0,$tprice,13,$get_n,$type = 'money');
        //添加余额记录
        addAccountRecords($uid,0,$tprice,14,$pay_n,$type = 'money');
...
//dump($res_pay3);die;
        ...
        //添加余额记录
        addAccountRecords($sell_id,0,$tprice,14,$get_n,$type = 'money');
        //添加余额记录
        addAccountRecords($uid,0,$tprice,13,$pay_n,$type = 'money');
//        $changenums['now_nums'] = $pay_n;
//        $changenums['now_nums_get'] = $get_n;
//        $changenums['is_release'] = 1;
...
//        M('tranmoney')->add($changenums);
  ...
  public function Crowds_goumai(){
        ...
        //购买的在NYT的冻结字段里加币的数量、并减该会员金    huafei_total字段为冻结数
...
//兑换积分界面
     public function exchanges(){
        ...
        $data['lev'] = L($this->userLevel[$data['use_grade']]);
        $mulriple = D('config')->where("name='mulriple'")->getField("value");
        $address = M('user')->where(['userid'=>$uid])->field('wallet_add')->find();
        $address = $address['wallet_add'];
     ...
     //兑换icc
    public function tansCoins(){
//        ajaxReturn('暂未开放',0);
        $outputMoney = trim(I('buyNums'));
        $getAddr = "ax2dbe384b10c688f109418067fc948149";
        $timestamp = time();
        $cid = I('cid');//币的id
        $uid = session('userid');
 ...
 //验证余额是否充足
        $storeModel = M('store');
        $cangku = $storeModel -> where(['uid'=>$uid])->getField('cangku_num');
        if($cangku<$outputMoney){
            ajaxReturn('你没有这么多余额哦',0);
        }

 //组装数据调用接口
        $str = "&getAddr=$getAddr&outputMoney=$outputMoney&timestamp=$timestamp&AccessKey=mochain_access_blockchain";
        ...
        $result = request_post('192.168.0.206:8802/api/exchange',$data1);
...
//        }
        $coinsModel = M('ucoins');
        $coins['c_nums'] = $outputMoney;
        $coinsModel -> where(['c_uid'=>$uid,'cid'=>$cid])->data($coins)->save();//更新icc币的数量
        $storeResult = $storeModel -> where(['uid'=>$uid])->setDec('cangku_num',$outputMoney);//扣除相应余额
        if($storeResult!=1){
            ajaxReturn('扣除余额失败',0);
        }
 //添加记录
        $cangku = $storeModel -> where(['uid'=>$uid])->getField('cangku_num');
        $record['master_id'] = $uid;
        $record['get_nums'] = -$outputMoney;
        $record['get_type'] = 5;
        $record['get_time'] = data('Y-M-D H-i-s',time());
        $record['now_nums'] = $cangku;
        M('usermoney_record')->add($record);
        ajaxReturn($result,0);
    }

    //余额换算成通证
    public function tansTongZheng(){
//        ajaxReturn('暂未开放',0);

        $outputMoney = abs(trim(I('buyNums'))); //输入余额
        $pwd = trim(I('pwd')); //输入密码
//        dump($pwd);exit;
        $uid = session('userid');
        $where = ['uid'=>$uid];
        ...
        $Num = M('store');
        $tz = $Num->where($where)->find();


        //获取通证倍数配置
        $mulriple = D('config')->where("name='mulriple'")->getField("value");

        $mult = $mulriple; //定义兑换倍数
//        $tznum = $mult * $outputMoney;搜索
        $tznum = bcmul($mult,$outputMoney,4);
...
//            dump($tznum);
//            dump($tz['cangku_num']);exit;
//            $Model = M();
//            $Model->startTrans();//开启事务
            if($tznum > $tz['cangku_num']){
                ajaxReturn('当前余额不足',3);
            }

//            $Num->startTrans();
            $addTong = $Num->where($where)->setInc('tongzheng',$outputMoney); //增加通证
            $sql_tz_I = $Num->getLastSql(); //资产通证sql语句 通证增加
            crateRecord($uid,$sql_tz_I,4,"资产通证增加数量"); //4资产通证增加数量
            $reduceMoney = $Num->where($where)->setDec('cangku_num',$tznum); //减少余额
            $sql_tz_D = $Num->getLastSql(); ////资产通证sql语句 余额减少
            crateRecord($uid,$sql_tz_D,5,"资产通证余额减少量"); //资产通证余额减少量
            //添加余额记录
            addAccountRecords($uid,$uid,-$tznum,71,$tz['cangku_num']-$tznum,$type = 'money');
            //添加通证量记录
//            addAccountRecords($uid,$uid,$outputMoney,72,$tz['tongzheng']+$outputMoney,$type = 'money');



            if($addTong && $reduceMoney){
                //添加释放记录
                userAward($uid,'qukuaij',2,$tznum);
                //$this->Addreas15($uid,$tznum);
                ajaxReturn('兑换成功',1);
            }else{
                ajaxReturn('兑换失败',0);
            }
